...
export function createAdminClient() {
  ...
  // Ce message est lu par un patron, pas par un développeur : il faut qu'il
  // puisse être répété tel quel à la personne qui tient le déploiement.
  if (!key) {
    throw new Error('Configuration incomplète : la clé de service Supabase '
      + '(SUPABASE_SERVICE_ROLE_KEY) manque sur le serveur.');
  }
